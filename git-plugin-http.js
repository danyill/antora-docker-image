...
function mergeHeaders(headers, extraHeaders) {
  ...
  for (const name in extraHeaders)
    mergedHeaders[name.toLowerCase()] = extraHeaders[name];
...
module.exports = ({
  ...
  if ((httpsProxy || httpProxy) && noProxy !== "*") {
    ...
    return {
      async request({ url, method, headers, body }) {
        ...
        const agent =
          proxy.url && shouldProxy(url, { no_proxy: noProxy })
            ? new proxy.Agent({ proxy: proxy.url })
            : undefined;
        return new Promise((resolve, reject) =>
          get({ url, method, agent, headers, body }, (err, res) =>
            err ? reject(err) : resolve(distillResponse(res))
          )
        );
      },
    };
  ...
  return {
    async request({ url, method, headers, body }) {
      headers = Object.assign(mergeHeaders(headers, extraHeaders), {
        connection: "close",
      });
      body = await mergeBuffers(body);
      return new Promise((resolve, reject) => {
