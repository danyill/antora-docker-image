...
We tag the image as latest and the next version:
  ...
  $ docker tag danyill/antora:custom danyill/antora:0.10
  $ docker tag danyill/antora:custom danyill/antora:latest

Together:

  $ docker tag danyill/antora:custom danyill/antora:0.10 && docker tag danyill/antora:custom danyill/antora:latest
...
We push a `latest` tag and the version (current increments are `0.1`):
  ...
  $ docker push danyill/antora:0.10 # or whatever your version
  $ docker push danyill/antora:latest

or 

  $ docker push danyill/antora:0.10 && docker push danyill/antora:latest
